import type { Response } from "express";
import type { AuthedRequest } from "../middleware/clerkAuth.js";
import type { CategoryRepository } from "../repositories/categoryRepository.js";
import { createCategorySchema } from "../models/schemas.js";

export function categoryController(repo: CategoryRepository) {
  return {
    list: async (req: AuthedRequest, res: Response) => {
      const rows = await repo.list(req.userId);
      res.json(rows);
    },
    create: async (req: AuthedRequest, res: Response) => {
      const parsed = createCategorySchema.safeParse(req.body);
      if (!parsed.success) {
        res.status(400).json({ error: parsed.error.flatten() });
        return;
      }
      try {
        const row = await repo.create(req.userId, parsed.data);
        res.status(201).json(row);
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : "Failed";
        if (msg.includes("duplicate") || msg.includes("unique")) {
          res.status(409).json({ error: "Category title already exists" });
          return;
        }
        throw e;
      }
    },
  };
}
